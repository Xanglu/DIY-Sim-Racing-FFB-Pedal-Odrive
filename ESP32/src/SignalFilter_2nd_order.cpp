...
float KalmanFilter_2nd_order::filteredValue(float observation, float command, uint8_t modelNoiseScaling_u8) {
  ...
  if (fabsf(S_cov) > 0.000001f) {
    ...
    // temp_P = (I - K*H) * P_pred
    //
    // temp_P[0][0] = (1.0f - _K[0]) * P_pred[0][0] - _K[0] * P_pred[1][0] - _K[0] * P_pred[2][0];
    // temp_P[0][1] = (1.0f - _K[0]) * P_pred[0][1] - _K[0] * P_pred[1][1] - _K[0] * P_pred[2][1];
    // ...
    //
    // Second term: K*R*K'
    ...
    temp_P[0][0] = (1.0f - _K[0]) * P_pred[0][0] - _K[0] * P_pred[1][0] - _K[0] * P_pred[2][0];
    temp_P[0][1] = (1.0f - _K[0]) * P_pred[0][1] - _K[0] * P_pred[1][1] - _K[0] * P_pred[2][1];
    temp_P[0][2] = (1.0f - _K[0]) * P_pred[0][2] - _K[0] * P_pred[1][2] - _K[0] * P_pred[2][2];

    temp_P[1][0] = -_K[1] * P_pred[0][0] + (1.0f - _K[1]) * P_pred[1][0] - _K[1] * P_pred[2][0];
    temp_P[1][1] = -_K[1] * P_pred[0][1] + (1.0f - _K[1]) * P_pred[1][1] - _K[1] * P_pred[2][1];
    temp_P[1][2] = -_K[1] * P_pred[0][2] + (1.0f - _K[1]) * P_pred[1][2] - _K[1] * P_pred[2][2];

    temp_P[2][0] = -_K[2] * P_pred[0][0] - _K[2] * P_pred[1][0] + (1.0f - _K[2]) * P_pred[2][0];
    temp_P[2][1] = -_K[2] * P_pred[0][1] - _K[2] * P_pred[1][1] + (1.0f - _K[2]) * P_pred[2][1];
    temp_P[2][2] = -_K[2] * P_pred[0][2] - _K[2] * P_pred[1][2] + (1.0f - _K[2]) * P_pred[2][2];
